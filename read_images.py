...
def read_bmps(full_path, bmp_files):
    ...
    for bmp_file in bmp_files:
        ...
        image_path = os.path.join(full_path, bmp_file)
...
relative_path = 'MelanomaClassifier/PH2Dataset/Custom Images/Normal'
current_directory = os.getcwd()
full_path = os.path.join(current_directory, relative_path)
full_path = os.path.abspath(full_path)

# Get a list of all bmp files in the folder
bmp_files = [file for file in os.listdir(full_path) if file.lower().endswith('.bmp')]

BW_images = read_bmps(full_path, bmp_files)[0]
BW_images_arrays = read_bmps(full_path, bmp_files)[1] 
...
try:
    ...
    print(f"Permission error accessing the directory '{full_path}'.")
except Exception as e:
    print(f"An error occurred: {e}")

images = read_bmps(full_path, bmp_files)[0]
images_arrays = read_bmps(full_path, bmp_files)[1]
