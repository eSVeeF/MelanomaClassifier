...
def calculate_orientation(lesion_mask):
    """
    Calculate the orientation of a lesion from its binary mask.
...
def calculate_asymmetry_index(lesion_mask):
    """
    Calculate the asymmetry index of a lesion from its binary mask (across minor axis).
    ...
    # Flip the lesion across the minor axis (horizontal flip)
    flipped_lesion = np.fliplr(rotated_lesion_binary)
